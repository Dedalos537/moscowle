...
def add_patient():
    ...
    if is_full_account:
        ...
        if email_sent:
            flash(f'Paciente {new_patient.username} agregado.<br>'
                  f'Email: <strong>{email}</strong><br>'
                  f'Contraseña: <strong>{password}</strong><br>'
                  f'<small>El paciente recibe las credenciales por correo.</small>', 'success')
        else:
            flash(f'Paciente {new_patient.username} agregado.<br>'
                  f'No se pudo enviar el correo.<br>'
                  f'Email: <strong>{email}</strong><br>'
                  f'Contraseña: <strong>{password}</strong><br>'
                  f'<small>Comparte estas credenciales con el paciente.</small>', 'warning')
    else:
        flash(f'Paciente {new_patient.username} creado (presencial/sin email).<br>'
              f'<small>Registro creado para gestionar terapias y pagos.</small><br>'
              f'<small>La cuenta no tiene acceso al sistema. Para activarla, edita el perfil y añade un email.</small>', 'success')
...
def api_analytics():
    ...
    if my_patient_ids:
        ...
        # Difficulty matrix
        games = db.session.query(SessionMetrics.game_name).filter(
            SessionMetrics.user_id.in_(my_patient_ids)
        ).distinct().all()
        dm = []
        for (game,) in games:
            levels_q = db.session.query(
                SessionMetrics.prediction,
                func.avg(SessionMetrics.accurracy),
                func.count(SessionMetrics.id)
            ).filter(
                SessionMetrics.user_id.in_(my_patient_ids),
                SessionMetrics.game_name == game
            ).group_by(SessionMetrics.prediction).all()
            levels = []
            label_map = {0: 'Mantener', 1: 'Avanzar', 2: 'Apoyo'}
            for pred, acc, cnt in levels_q:
                if pred is not None:
                    levels.append({'level': label_map.get(pred, str(pred)), 'accuracy': round(float(acc), 1), 'count': cnt})
        ...
        # Recent adaptations
        recent = db.session.query(SessionMetrics, User).join(
            User, SessionMetrics.user_id == User.id
        ).filter(
            SessionMetrics.user_id.in_(my_patient_ids)
        ).order_by(SessionMetrics.date.desc()).limit(10).all()
...
def api_reports_overview():
    ...
    if my_patient_ids:
        ...
        avg_session_time = round(db.session.query(func.avg(SessionMetrics.avg_time))\
            .filter(SessionMetrics.user_id.in_(my_patient_ids)).scalar() or 0, 1)
        completed_objectives = SessionMetrics.query.filter(
            SessionMetrics.user_id.in_(my_patient_ids),
            SessionMetrics.accurracy >= 80
        ).count()
