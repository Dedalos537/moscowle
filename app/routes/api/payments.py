...
def mark_payment_paid(payment_id):
    ...
    return jsonify(
        {'success': True, 'message': f'Pago de {payment.amount} registrado exitosamente.', 'payment_id': payment.id}
    )
...
def send_payment_reminder():
    ...
    try:
        ...
        if channel == 'email':
            ...
            subject = 'Recordatorio: pago pendiente - Centro de Terapias'
            body = f"""
Hola {patient_name},
...
"""
            try:
                ...
                return api_response(
                    success=True, data={'message': f'Recordatorio enviado a {patient_email}', 'channel': 'email'}
                )
            except Exception as e:
                current_app.logger.error(f'Error sending email reminder: {e}')
                return api_response(
                    success=False, error={'message': f'Error al enviar recordatorio por email: {str(e)}'}, status=500
                )
            ...
            if not sms_service.is_available():
                return jsonify(
                    {'success': False, 'error': 'Servicio SMS/WhatsApp no disponible. Configure Twilio credentials.'}
                ), 501
            ...
            return api_response(
                success=False,
                error={'message': f'Canal no soportado: {channel}. Use email, sms, o whatsapp.'},
                status=400,
            )
...
def search_patients():
    ...
    result = []
    for p in patients:
        result.append({'id': p.id, 'username': p.username, 'email': p.email, 'phone': getattr(p, 'phone', '')})
