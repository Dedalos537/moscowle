...
from app.models import User, Payment, AIConversation, AIChatMessage, Appointment
...
def _handle_create_user(params):
    ...
    if not name:
        return {'response': "Proporciona el nombre del nuevo usuario.", 'action_result': {'status': 'validation_failed', 'error': 'name_required'}}
    ...
    try:
        ...
        if existing:
            return {'response': f"Ya existe un usuario con nombre similar: {existing.username}", 'action_result': {'status': 'duplicate'}}

        email = f"{name.lower().replace(' ', '.')}@centrojuanpabloii.com"
        new_user = User(
            username=name, email=email,
            password=generate_password_hash('changeme123'),
            role='jugador', is_active=True
        )
        ...
        db.session.rollback()
        return {'response': f"Error al crear usuario: {str(e)[:60]}", 'action_result': {'status': 'error', 'error': str(e)}}


def _handle_create_appointment(params):
    patient_name = params.get('patient_name', '')
    if not patient_name:
        return {'response': "Indica el nombre del paciente y el dia/hora para la sesion.", 'action_result': {'status': 'validation_failed', 'error': 'missing_patient'}}

    patient = _find_patient_by_name(patient_name)
    if not patient:
        return {'response': f"No encontre al paciente '{patient_name}'.", 'action_result': {'status': 'patient_not_found'}}

    try:
        start = datetime.utcnow() + timedelta(hours=1)
        end = start + timedelta(hours=1)
        appt = Appointment(
            therapist_id=current_user.id, patient_id=patient.id,
        ...
        db.session.rollback()
        return {'response': f"Error al crear sesion: {str(e)[:60]}", 'action_result': {'status': 'error', 'error': str(e)}}


def _handle_assign_therapist(params):
    patient_name = params.get('patient_name', '')
    if not patient_name:
        return {'response': "Indica el nombre del paciente para asignarle un terapeuta.", 'action_result': {'status': 'validation_failed', 'error': 'missing_patient_name'}}
    ...
    if not therapist:
        return {'response': "No hay terapeutas disponibles para asignar.", 'action_result': {'status': 'no_therapist_available'}}

    patient.assigned_therapist_id = therapist.id
    db.session.commit()
    return {'response': f"Terapeuta {therapist.username} asignado a {patient.username}.", 'action_result': {'status': 'success', 'therapist_id': therapist.id}}


def _handle_update_session(params):
    patient_name = params.get('patient_name', '')
    if not patient_name:
        return {'response': "Indica el nombre del paciente para actualizar su sesion.", 'action_result': {'status': 'validation_failed'}}
    ...
    try:
        appt = Appointment.query.filter_by(patient_id=patient.id, status='scheduled').order_by(Appointment.start_time).first()
        if not appt:
            return {'response': f"{patient.username} no tiene sesiones programadas.", 'action_result': {'status': 'no_sessions'}}
        ...
        return {'response': f"Sesion de {patient.username} reprogramada.", 'action_result': {'status': 'success', 'appointment_id': appt.id}}
    except Exception as e:
...
def _handle_delete_session(params):
    patient_name = params.get('patient_name', '')
    if not patient_name:
        return {'response': "Indica el nombre del paciente para cancelar su sesion.", 'action_result': {'status': 'validation_failed'}}
    ...
    try:
        appt = Appointment.query.filter_by(patient_id=patient.id, status='scheduled').order_by(Appointment.start_time).first()
        if not appt:
            return {'response': f"{patient.username} no tiene sesiones programadas para cancelar.", 'action_result': {'status': 'no_sessions'}}

        appointment_service.delete_session(appt.id, current_user.id)
        return {'response': f"Sesion de {patient.username} cancelada.", 'action_result': {'status': 'success', 'appointment_id': appt.id}}
    except Exception as e:
        return {'response': f"Error: {str(e)[:60]}", 'action_result': {'status': 'error', 'error': str(e)}}
...
def _handle_broadcast(user_message):
    try:
        ...
        for p in patients:
            ...
            count += 1
        return {'response': f"Mensaje enviado a {count} pacientes.", 'action_result': {'status': 'success', 'recipients': count}}
    except Exception as e:
        return {'response': f"Error al enviar mensajes: {str(e)[:60]}", 'action_result': {'status': 'error', 'error': str(e)}}
...
def send_message():
    ...
    try:
        ...
        save_chat_message(
            ...
            intent=intent, parameters=params if isinstance(params, dict) else {},
...
def upload_voucher():
    ...
    try:
        ...
        try:
            ...
            analysis_result = {
                'image_type': 'generic', 'amount': None, 'confidence': 0.3,
                'text_extracted': '', 'sections': {}, 'raw_text': '', 'ocr_available': False,
            }
        ...
        save_chat_message(
            ...
            parameters={'amount': amount, 'image_type': analysis_result['image_type'],
                        'confidence': confidence, 'filepath': filepath,
        ...
        return jsonify({
            ...
            'extracted': {'amount': amount, 'image_type': analysis_result['image_type'],
                          'confidence': confidence, 'status': action_status},
            'filepath': filepath, 'conversation_id': conversation_id,
...
def confirm_payment():
    ...
    try:
        ...
        payment_service.register_payment(
            patient_id=patient.id, amount=amount, method='IA/Copilot + OCR',
