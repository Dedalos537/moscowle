...
async def get_active_users():
    """Terapeutas activos con Async DAO + selectinload"""
    try:
...
async def get_upcoming_appointments():
    ...
    try:
        ...
        async with get_async_db() as session:
            ...
            data = [
                {'id': a.id, 'title': a.title, 'start': a.start_time.isoformat(), 'status': a.status}
...
async def get_user_metrics(user_id):
    """Métricas del usuario con async"""
    try:
        async with get_async_db() as session:
            ...
            data = {
                ...
                'recent_sessions': [
                    {'id': m.id, 'game': m.game_name, 'accuracy': m.accurracy, 'date': m.date.isoformat()}
                    for m in metrics
                ],
            }
