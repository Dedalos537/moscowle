# SMS y WhatsApp: pywhatkit (gratis) o Twilio
import os
...
class SMSWhatsAppService:
    ...
    def _send_whatsapp_pywhatkit(self, phone_number, message_text):
        ...
        try:
            ...
            # Calculate send time (1 minute from now)
            now = datetime.now()
            send_hour = now.hour
            ...
            # pywhatkit will open Chrome and WhatsApp Web automatically
            pywhatkit.sendwhatmsg(
                phone_number=f"+{clean_phone}",
                message=message_text,
                time_hour=send_hour,
                time_min=send_minute,
                wait_time=15,  # Wait 15 seconds for confirmation
...
Centro de Terapias"""
        ...
        # Try pywhatkit first (FREE)
        if self.pywhatkit_available:
            thread = threading.Thread(
                target=self._send_whatsapp_pywhatkit,
                args=(phone_number, message_body),
                daemon=True
            )
            thread.start()
            current_app.logger.info(f" WhatsApp via pywhatkit scheduled (background thread)")
            return True
...
Centro de Terapias"""
        ...
        if self.pywhatkit_available:
            ...
            thread.start()
            current_app.logger.info(f" Confirmation WhatsApp via pywhatkit scheduled")
            return True
