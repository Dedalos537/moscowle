...
from app.models import Appointment, Expense, Payment, Sede, User
...
class ContextLoader:
    ...
    def get_active_patients_context() -> dict:
        ...
        context = {'total': len(patients), 'patients': []}
        ...
        for p in patients:
            ...
            last_payment = Payment.query.filter_by(patient_id=p.id).order_by(Payment.date.desc()).first()
    ...
    def get_financial_context() -> dict:
        ...
        return {
            ...
            'total_expenses': float(
                ...
                .filter(db.func.year(Expense.date) == today.year, db.func.month(Expense.date) == today.month)
                .scalar()
                or 0
            ),
            'total_payments': float(
                db.session.query(db.func.sum(Payment.amount))
                .filter(db.func.year(Payment.date) == today.year, db.func.month(Payment.date) == today.month)
                .scalar()
                or 0
            ),
        }
    ...
    def get_sessions_context() -> dict:
        ...
        sessions = Appointment.query.filter(
            Appointment.start_time >= this_week_start, Appointment.start_time <= this_week_end
        ).all()

        sessions_by_status = {'pending': [], 'confirmed': [], 'completed': [], 'cancelled': []}
    ...
    def get_sedes_context() -> dict:
        ...
        for sede in sedes:
            ...
            sede_data = {'id': sede.id, 'name': sede.name, 'address': sede.address, 'active_patients': patients}
    ...
    def get_therapists_context() -> dict:
        ...
        context = {'total': len(therapists), 'therapists': []}

        for therapist in therapists:
            assigned_patients = User.query.filter_by(assigned_therapist_id=therapist.id, is_active=True).count()
    ...
    def get_full_context() -> dict:
        ...
        try:
            ...
            logger.info(
                f'Context loaded: {context["patients"]["total"]} patients, '
