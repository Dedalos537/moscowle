...
class AppointmentService:
    def __init__(self):
        ...
        self.display_timezone = 'America/Lima'

    def validate_session_times(
        self, start_time, end_time, patient_id, therapist_id, session_id=None, ignore_therapist_conflict=False
    ):
    ...
    def get_all_appointments(self, start_dt, end_dt):
        return Appointment.query.filter(Appointment.start_time >= start_dt, Appointment.start_time <= end_dt).all()
    ...
    def get_patient_appointments(self, patient_id, start_dt=None, end_dt=None, limit=10):
        ...
        if start_dt and end_dt:
            return (
                query.filter(Appointment.start_time >= start_dt, Appointment.start_time <= end_dt)
                .order_by(Appointment.start_time.asc())
                .all()
            )
        else:
            ...
            return (
                query.filter(Appointment.start_time >= today_start)
                .order_by(Appointment.start_time.asc())
                .limit(limit)
                .all()
            )
    ...
    def create_session(self, therapist_id, data, therapist_username):
        ...
        try:
            ...
            self.notification_service.create_notification(
                therapist_id, f'Sesión programada: {appt.title} — {fmt_short}', url_for('therapist.sessions')
            )
            self.notification_service.create_notification(
                patient_id,
            ...
            self.email_service.send_session_notification(
                patient.email, patient.username, 'programada', msg_details, 'patient'
            )
            self.email_service.send_session_notification(
                therapist.email,
                therapist.username,
    ...
    def update_session(self, session_id, data):
        ...
        if 'attendance' in data:
            ...
            if patient:
                if patient.sessions_attended is None:
                    patient.sessions_attended = 0
    ...
    def delete_session(self, session_id, therapist_id):
        ...
        try:
            self.notification_service.create_notification(
                therapist_id, f'Sesión eliminada: {title}', link=url_for('therapist.sessions')
            )
            self.notification_service.create_notification(
                patient_id, f'Tu sesión programada ({title}) ha sido cancelada.', link=url_for('patient.calendar')
            )
            ...
            def _send_emails():
                with current_app.app_context():
                    try:
                        ...
                        if patient:
                            self.email_service.send_session_notification(
                                patient.email, patient.username, 'cancelada', details
                            )
                        if therapist:
                            self.email_service.send_session_notification(
                                therapist.email, therapist.username, 'cancelada', details
                            )
                    except Exception:
                        pass
    ...
    def set_session_games(self, session_id, game_filenames):
        ...
        from app.models import AppointmentGame, Game
        ...
        for filename in game_filenames:
            ...
            if not game:
                ...
                game = Game(
                    title=filename.replace('.html', '').replace('_', ' ').title(), filename=filename, is_active=True
                )
                db.session.add(game)
                db.session.flush()
    ...
    def transition_status(self, session_id, new_status, changed_by_user_id=None, notify=True):
        ...
        try:
            ...
            current_app.logger.info(f'Sesión {session_id}: {old_status} → {new_status} (por {change_by})')
    ...
    def _send_status_change_notification(self, appt, old_status, new_status):
        ...
        try:
            ...
            self.notification_service.create_notification(
                user_id=appt.patient_id, message=message, type='session_update'
            )
