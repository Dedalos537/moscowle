# Procesa reportes Yape con idempotencia, deduplicación y auditoría
...
class YapeService:
    ...
    def parse_yape_excel(self, file_stream):
        ...
        try:
            # Forzar lectura completa para evitar problemas con iter_rows en archivos mal formados
            workbook = openpyxl.load_workbook(file_stream, data_only=True)
            worksheet = workbook.active
            
            # 1. Obtener todas las filas de la hoja de forma densa
            # A veces worksheet.max_column reporta 1 pero hay más datos en celdas individuales
            raw_data = []
            max_col = 1
            ...
            # 2. Identificar la fila del encabezado escaneando todas las filas cargadas
            header_row_idx = -1
            headers = []
            ...
            if header_row_idx == -1:
                # Si falló la detección por keywords, intentar la primera que tenga al menos 4 columnas con algo
                for i, row in enumerate(raw_data):
    ...
    def _map_csv_row(self, row):
        ...
        operation_number = self._extract_field(normalized_row, [
            ...
            'id', 'referencia', 'reference', 'num_operacion', 'nro de operacion',
            'nro operacion', 'nro. operacion'
        ])
        
        transaction_date = self._extract_field(normalized_row, [
            'fecha de operación', 'fecha', 'date', 'transaction_date', 'fecha transaccion',
            'fecha de operacion', 'fecha de operaciã³n', 'fecha operacion', 'fecha de operación'
        ])
        ...
        sender_name = self._extract_field(normalized_row, [
            ...
            'from', 'quien', 'de', 'remitente', 'razon social', 'nombre_emisor',
            'emisor', 'contacto'
        ])
        ...
        if not sender_name or str(sender_name).strip() == "":
            sender_name = self._extract_field(normalized_row, [
                'destino', 'to', 'para', 'beneficiario', 'nombre_receptor', 'receptor'
            ])
        
        amount = self._extract_field(normalized_row, [
            'monto', 'amount', 'cantidad', 'importe', 'valor', 'monto total',
            'valor transaccion'
        ])
        message = self._extract_field(normalized_row, [
            'mensaje', 'description', 'nota', 'message', 'concepto',
            'tipo de transacción', 'tipo de transaccion', 'tipo', 'comentario',
            'referencia'
        ])
        
        #  GENERAR operation_number si no existe 
        if not operation_number or str(operation_number).strip().lower() in ['none', '', 'nan']:
            # Evitar colisiones usando un hash o combinando datos si no hay ID oficial
            ...
            # Formato de la imagen usa "Fecha de operación" ej: 21/03/2026 17:05:05
    ...
    def _categorize_transaction(self, transaction_data):
        ...
        # 2. IA para automatización (Opcional si hay cuota)
        # Solo usar IA para mensajes largos o complejos
        if len(message) > 10:
            try:
                ...
                # Si falla por cuota o formato, dejamos que siga al fallback
                logger.debug(f"IA omitida o con error para '{message}': {e}")
            
        # 3. Fallback simple (Reglas de negocio)
