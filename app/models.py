...
class User(db.Model, UserMixin):
    ...
    account_status = db.Column(db.String(50), default='active')  # active, inactive, retired, debtor
...
class AIConversation(db.Model):
    """Modelo para guardar el historial de conversaciones con el Copilot Llama."""
...
class AIChatMessage(db.Model):
    """Modelo para cada mensaje en una conversación con la IA."""
...
class SessionAudit(db.Model):
    ...
    # ── FASE 4: Feedback del Terapeuta ──
    feedback_engagement = db.Column(db.Integer, nullable=True)  # 1-5
    feedback_progress = db.Column(db.Integer, nullable=True)     # 1-5
