...
def register_request_handlers(app):
    """Handlers de CORS y before/after request"""
    ...
    def before_request():
        ...
        if request.method == 'OPTIONS':
            ...
            if origin in allowed_origins:
                ...
                response.headers['Access-Control-Allow-Origin'] = 'https://moscowle.centrojuanpabloii.com'
            response.headers['Access-Control-Allow-Methods'] = 'GET, POST, PUT, DELETE, OPTIONS'
            response.headers['Access-Control-Allow-Headers'] = 'Content-Type, X-App-Key, Authorization, X-CSRFToken'
        ...
        if request.path.startswith('/api/') or request.path.startswith('/admin/api/'):
            # Saltar validacion pa: webhooks, auth, health, OPTIONS, sesiones activas
            skip_appkey = (
            ...
            try:
                ...
                with db.engine.connect() as conn:
                    if 'title' not in columns:
                        conn.execute(text("ALTER TABLE notification ADD COLUMN title VARCHAR(200) DEFAULT NULL"))
                        app.logger.info("Auto-migration: added notification.title")
                    if 'type' not in columns:
                        conn.execute(text("ALTER TABLE notification ADD COLUMN type VARCHAR(50) DEFAULT 'info'"))
                        app.logger.info("Auto-migration: added notification.type")
                    conn.commit()
            except Exception:
