...
# Extract the header part up to {% block content %}
match_header = re.search(r'(.*?{% block content %}\n)', content, re.DOTALL)
pre_content = match_header.group(1)

# Extract everything from the <footer> up to the end including {% endblock %} and {% block extra_scripts %}
match_footer = re.search(r'(\n    <footer id="footer".*)', content, re.DOTALL)
post_content = match_footer.group(1)

full_new_file = pre_content + new_content + post_content
with open(dashboard_path, "w", encoding="utf-8") as f:
    f.write(full_new_file)

print("Updated successfully without breaking Jinja blocks")
